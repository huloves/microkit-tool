...
namespace util {
	uint64_t msb(uint64_t x) {
		// Handle zero case explicitly since clz(0) is undefined or returns total bits.
		if (x == 0) return 0;

		// 64 - number of leading zeros - 1 to convert to position, 0-indexed
		return 64 - __builtin_clzll(x) - 1;
	}

	uint64_t lsb(uint64_t x) {
		// If x is zero, __builtin_ctzll is undefined.
		if (x == 0) 
			return 0;
